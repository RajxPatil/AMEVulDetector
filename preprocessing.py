...
def get_pattern_feature():
    ...
    pattern_feature_train_path = "./pattern_feature/feature_FNN/reentrancy/train/"
    pattern_feature_valid_path = "./pattern_feature/feature_FNN/reentrancy/validation/"

    final_pattern_feature_train = []  # pattern feature train
    pattern_feature_train_label_path = "./pattern_feature/label_by_extractor/reentrancy/train/trainlabels.txt"

    final_pattern_feature_test = []  # pattern feature test
    pattern_feature_test_label_path = "./pattern_feature/label_by_extractor/reentrancy/validation/validlabels.txt"

    f_train = open(train_total_name_path, 'r')
    lines = f_train.readlines()
    for line in lines:
        line = line.strip('\n').split('.')[0]
        tmp_feature = np.loadtxt(pattern_feature_train_path + line + '.txt')
    ...
    for line in lines:
        line = line.strip('\n').split('.')[0]
        tmp_feature = np.loadtxt(pattern_feature_valid_path + line + '.txt')
...
def get_graph_feature():
    graph_feature_train_data_path = "./graph_train_features.txt"
    graph_feature_train_label_path = "./graph_feature/reentrancy/label_by_experts_train.txt"

    graph_feature_test_data_path = "./graph_valid_features.txt"
