...
# [Your existing variable definitions and helper functions remain unchanged]

# Function to generate and save graph features
      # Add fallback node if necessary
def process_all_contracts_in_folder(folder_path, output_node_dir, output_edge_dir):
    sol_files = [f for f in os.listdir(folder_path) if f.endswith('.sol')]
    for sol_file in sol_files:
        sol_file_path = os.path.join(folder_path, sol_file)
        node_feature, edge_feature = generate_graph(sol_file_path)
        ...
        # Define output file names based on the .sol file name
        name = os.path.splitext(sol_file)[0]  # Remove '.sol' extension
        node_output_path = os.path.join(output_node_dir, f"{name}_nodes.csv")
        edge_output_path = os.path.join(output_edge_dir, f"{name}_edges.csv")
        
        # Save Node Features to CSV
        with open(node_output_path, 'w', newline='', encoding='utf-8') as nf:
            writer = csv.writer(nf)
            writer.writerow(["Node_ID", "Type", "Related_Node", "Attributes", "Flag", "Label"])  # Header
            for node in node_feature:
                writer.writerow(node)
        
        # Save Edge Features to CSV
        with open(edge_output_path, 'w', newline='', encoding='utf-8') as ef:
            writer = csv.writer(ef)
            writer.writerow(["Source", "Target", "Edge_Type", "Edge_Feature"])  # Header
            for edge in edge_feature:
                writer.writerow(edge)
        
        print(f"Saved graph features for {sol_file} to {node_output_path} and {edge_output_path}")


if __name__ == "__main__":
    # Define source and output directories
    source_code_folder = "../data_example/reentrancy/source_code/train/"
    output_node_dir = "../data_example/reentrancy/node/train/"
    output_edge_dir = "../data_example/reentrancy/edge/train/"
    
  
    
    # Process all .sol files in the source directory
    process_all_contracts_in_folder(source_code_folder, output_node_dir, output_edge_dir)
